...
public:
    ...
    //connPool是数据库连接池指针
    threadpool(int actor_model,connection *connPool,int thread_number = 8,int max_request = 10000);
    ~threadpool();
...
threadpool<T>::threadpool(int actor_model,connection_pool *connPool,int thread_number,int max_request):
                        ...
                        m_max_requests(max_request),m_threads(NULL),m_connPool(connPool)
{
    ...
    while(run)
    {
        ...
        if(1 == m_actor_model)
        {
            if(0 == request->m_state)
            {
                ...
                {
                    request->improv =  1;
                    connectionRALL msqlcon(&request->mysql,m_connPool);
                }
                else{
                    request->improv = 1;
